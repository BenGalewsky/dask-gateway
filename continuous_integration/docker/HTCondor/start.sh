...
await_startup() {
    i=0; while [ $i -ne 30 ]; do
        docker logs htc | grep "entered RUNNING state" | wc -l 2>/dev/null | grep --silent "2" \
            && start_script_finishing=true && break \
            || start_script_finishing=false && sleep 1 && i=$((i + 1)) && echo "Waiting for HTC container startup ($i seconds)"
    done
